...
class M_transaksi extends CI_Model {
	 public function simpan_cart_db()
	 {
	 	for($i=0; $i<count($this->cart->contents()); $i++){
				...
				$stok = $this->db->where('id_kaos', $this->input->post('id_kaos')[$i])
								 ->get('kaos')
								 ->row()
								 ->stok;
				...
				$this->db->where('id_kaos', $this->input->post('id_kaos')[$i])
						 ->update('kaos', $updatestok);
		}
	 	...
	 	for ($i=0;$i<count($this->input->post('rowid')) ;$i++) {
	 		...
	 		'id_kaos' =>$this->input->post('id_kaos')[$i],
	 ...
	 {
	 	return $this->db->where('id_transaksi', $id_transaksi)
	 					->join('kaos','kaos.id_kaos=nota.id_kaos')
	...
	public function cek($id_kaos){

		$cek_stok = $this->db->where('id_kaos', $id_kaos)->get('kaos')->row()->stok;
