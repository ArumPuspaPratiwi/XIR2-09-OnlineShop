...
<div class="col-md-6">
	<table id="example" class="table table-hover table-striped">
		...
		<tbody>
			<?php $no=0; foreach($tampil_kaos as $kaos): $no++; ?>
			<tr>
				<td><?=$no?></td>
				...
				<?php if($kaos->stok>0): ?>
				<td><a href="<?=base_url('index.php/transaksi/addcart/'.$kaos->id_kaos)?>" class="btn btn-info">Pesan</a></td>
			<?php elseif($kaos->stok==0): ?>
	...
	<form action="<?=base_url('index.php/transaksi/simpan')?>" method="post">
		...
		<table class="table table-striped table-hover" blocked>
			<tr>
				...
				<input type="hidden" name="id_kaos[]" value="<?=$items['id']?>">
